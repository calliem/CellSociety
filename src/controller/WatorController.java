...
public class WatorController extends ComplexController {
	...
	public WatorController(Map<String, String> parameters) throws InstantiationException,
	...
	private void manipulateCell(GridData data) {
		...
		List<Integer[]> eligibleDestinations = makeEligible(data, type);// new
																		// ArrayList<Integer[]>();
		if (eligibleDestinations.isEmpty()) {
			data.getNewGrid()[data.getRow()][data.getCol()] = ((ReproducingCell) data
					.curCell()).ageOneChronon();
			// grid[r][c] = ((ReproducingCell) grid[r][c]).ageOneChronon();
		} else {
			Integer[] dest = getDestination(eligibleDestinations);
			data.getNewGrid()[dest[0]][dest[1]] = ((ReproducingCell) (data.curCell()))
					.ageOneChronon();
			// grid[dest[0]][dest[1]] = ((ReproducingCell)
			// (grid[r][c])).ageOneChronon();
			...
			data.getNewGrid()[data.getRow()][data.getCol()] = ((ReproducingCell) data
					.curCell()).reproducingResult();
			// grid[r][c] = ((ReproducingCell)
			// (grid[r][c])).reproducingResult();
		}
	}
	 ...
	 * if(type.equals(Strings.FISH_CELL)){ list = getNeighbors(data,
	 * Strings.EMPTY_CELL); } else{ list = getNeighbors(data,
	 * Strings.FISH_CELL); if(list.isEmpty()){ list = getNeighbors(data,
	 * Strings.EMPTY_CELL); } } return list; }
	 * 
	 * private List<Integer[]> getNeighbors(Cell[][] grid, int r, int c, String
	 * s, List<Integer[]> updatedCoordinates){ List<Integer[]> list = new
	 * ArrayList<Integer[]>(); for(int d = -1; d <= 1; d += 2){
	 * 
	 * neighborsFromTwoDirections(grid, r+d, c, list);
	 * neighborsFromTwoDirections(grid, r, c+d, list);
	 * 
	 * } for(Integer[] i : new ArrayList<Integer[]>(list)){ if
	 * (!(grid[i[0]][i[1]].toString().equals(s)) || contains(updatedCoordinates,
	...
	protected void cellUpdate(GridData data) throws InstantiationException,
		...
		if (deadShark(data.curCell())) {
			data.getNewGrid()[data.getRow()][data.getCol()] = ((SharkCell) data.curCell())
					.reproducingResult();
