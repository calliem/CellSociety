...
public class AgentCell extends Cell implements Updatable {
	...
	private boolean sameLocation(GridData data, Integer[] newLocation) {
		return data.getRow() == newLocation[Controller.X_COORD]
				&& data.getCol() == newLocation[Controller.Y_COORD];
	}
	...
	private Integer[] closestEligibleNeighbor(GridData data,
		...
		for (Integer[] coords : eligibleNeighbors) {
			double curDist = Math.pow(coords[Controller.X_COORD], 2)
					+ Math.pow(coords[Controller.Y_COORD], 2);
			if (curDist < minDist) {
				minDist = curDist;
				closest = coords;
			}
